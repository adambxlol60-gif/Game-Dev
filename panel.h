...
const int upgrade1BtnY = panelY + 280;
const int upgrade1BtnW = 180;
const int upgrade1BtnH = 40;

const int upgrade2BtnX = panelX + 30;
const int upgrade2BtnY = panelY + 330;
const int upgrade2BtnW = 180;
const int upgrade2BtnH = 40;
...
inline void drawTowerPanel(ALLEGRO_FONT* font, ALLEGRO_BITMAP* drakeBmp, int drakeBmpW, int drakeBmpH, ALLEGRO_BITMAP* weekndBmp, int weekndBmpW, int weekndBmpH, ALLEGRO_BITMAP* elonBmp, int elonBmpW, int elonBmpH, ALLEGRO_BITMAP* bankBmp, int bankBmpW, int bankBmpH, ALLEGRO_BITMAP* icemanBmp, int icemanBmpW, int icemanBmpH, Tower towers[], int selectedTowerIndex) {
    ...
    //tight crop only for drake - his PNG has a lot of empty padding around the character
    ...
    //zoom Elon's and Iceman's portraits a bit so they fill the panel like the other characters
    if (selectedTower.type == towerElon)   finalScale *= 1.4f;
    if (selectedTower.type == towerIceman) finalScale *= 2.1f;
