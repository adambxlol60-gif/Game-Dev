...
//this constant makes the maximum number of waves the game can have
const int maxWaves = 100;
//this is the maximum number of individual enemy spawns allowed in a single wave
const int maxSpawnsPerWave = 200;

//this struct holds the data for one single enemy that will be spawned
struct Spawn {
...
//the loadWaves reads waves.txt and fills the allWaves array
inline int loadWaves(const char* filename, Wave allWaves[], int maxW) {
    ...
    while (fgets(line, sizeof(line), file)) {
        //it skips comments or any blank lines
        if (line[0] == '#' || line[0] == '\n' || line[0] == '\r') continue;
        ...
        //it scanfs the 6 expected values from the line, it skips it if its wrong
